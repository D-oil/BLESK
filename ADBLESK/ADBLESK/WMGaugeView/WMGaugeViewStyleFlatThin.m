...
#define kNeedleWidth        0.15
#define kNeedleHeight       0.4
#define kNeedleScrewRadius  0.315
...
- (void)drawfoodNeedleOnLayer:(CALayer*)layer withColor:(UIColor *)color inRect:(CGRect)rect;
{
    ...
    //三角形path
    [needlePath moveToPoint:CGPointMake(rect.size.width/2 - kNeedleWidth / 4 * rect.size.width, 0)];
    [needlePath addLineToPoint:CGPointMake(rect.size.width/2 + kNeedleWidth / 4 * rect.size.width , 0)];
    [needlePath addLineToPoint:CGPointMake(FULLSCALE(kCenterX, kNeedleWidth /1.5))];
