...
//    }
    ...
    if (backgroundPointLayer == nil) {
        ...
        for (int i = 0 ; i <= 100  ; i++ ) {
            ...
            CGPoint point = [WMGaugeView calcCircleCoordinateWithCenter:CGPointMake(self.frame.size.width / 2 -(self.frame.size.width / 2)*0.01, self.frame.size.height /2) andWithAngle:startAngle andWithRadius:self.frame.size.width / 2 -(self.frame.size.width / 2)*0.02];
            UIBezierPath *path = [UIBezierPath bezierPathWithOvalInRect:CGRectMake(point.x, point.y, (self.frame.size.width / 2)*0.02, (self.frame.size.width / 2)*0.02)];
            [path closePath];
    ...
    if (backgroundPointLineLayer == nil) {
        ...
        for (int i = 0 ;  i <= numOfLinePoint; i++) {
            ...
            [path closePath];
            backgroundPointLineLayer.fillColor = RGB(251, 0, 18).CGColor;
